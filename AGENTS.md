...
- Prefer generated `internal/apperr` helpers for stable package-level error kinds. Define one small string sentinel type per package with const values and add `//go:generate go run ../../cmd/errgen -type ErrKind`; the generated `apperr_gen.go` file supplies `Error()` methods, `type Error = apperr.Error[ErrKind]`, and non-generic package-local `AppErrMsg`, `AppErrMsgf`, `AppErrWrap`, and `AppErrWrapf` helpers. Use `Msg` for context-only errors and `Wrap` when preserving a cause so `errors.Is` and `errors.As` work.
